...
def update_business(id, post, db: Session, current_user):
    ...
    business = (
        db.query(models.Business)
        .join(models.BusinessMember, models.BusinessMember.business_id == models.Business.business_id)
        .filter(models.Business.business_id == id)
        .first()
    )
    ...
    own_business = (
        ...
        .join(models.BusinessMember, models.BusinessMember.business_id == models.Business.business_id)
        .filter(
            models.Business.business_id == id,
            models.BusinessMember.member_id == current_user.member_id
        )
        .first()
    )

    if not own_business and current_user.role != models.RoleEnum.super_admin:
...
def delete_business(id, db:Session, current_user):
    ...
    user_own_business = business.filter(models.BusinessMember.business_id == current_user.member_id).first()
    
    if not user_own_business and current_user.role != models.RoleEnum.super_admin:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Unathorized to delete this businesse")
...
def get_business_key(business_id, db:Session, current_user):
    ...
    key = (
        db.query(models.Business.business_key)
        .join(models.BusinessMember, models.BusinessMember.business_id == models.Business.business_id)
        .filter(models.Business.business_id == business_id)
        .filter(models.BusinessMember.user_id == current_user.user_id)
        .first()
    )
    ...
    if not key:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
                            detail=f"NO key for bsuiness with the ID: {business_id}")
