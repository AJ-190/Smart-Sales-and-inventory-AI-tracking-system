...
def add_product(business_id: int, post: Productcreate, db: Session = Depends(database.get_db), current_user: models.Users = Depends(dependencies.get_current_user)):
...
def get_products( business_id: int, db: Session = Depends(database.get_db),current_user: models.Users = Depends(dependencies.get_current_user),
    limit: int = 10,
    skip: int = 0,
    search: Optional[str] = ""):
...
def update_product(business_id: int,id: int, post: ProductUpdate, db: Session = Depends(database.get_db), current_user: models.Users = Depends(dependencies.get_current_user)):
    return products_service.update_product(business_id, id, post, db, current_user)
        
@router.delete("/{business_id}/{id}", status_code=204)
def delete_product(business_id: int,id: int, db: Session = Depends(database.get_db), current_user: models.Users = Depends(dependencies.get_current_user)):
...
@router.get("/{business_id}/low_stock", response_model=list[ProductResponse])
def low_stock(business_id: int , db:Session = Depends(database.get_db), current_user:models.Users = Depends(dependencies.get_current_user)):
    return products_service.low_stock(business_id,db, current_user)
