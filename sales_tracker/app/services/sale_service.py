...
from sales_tracker.app.routers import sales
from fastapi import status, HTTPException, Depends, APIRouter
from sales_tracker.app.utils import dependencies
from sales_tracker.app import schemas, models
from sales_tracker.app.services.products_service import get_member
from datetime import timedelta, datetime, date
from sqlalchemy import func, cast, Date
from sales_tracker.app.services.sale_analytics import check_stock


def add_sale(post: schemas.SaleCreate, db: Session, current_user):
...
def get_sales(
    ...
    sales = (
        query
        .order_by(models.Sale.created_at.desc())  # ← latest sales first
        .limit(limit)
        .offset(skip)
        .all()
    )
...
def delete_sale(id, db:Session, current_user):
    if current_user.role not in [
        models.RoleEnum.admin,
        models.RoleEnum.super_admin
    ]:
